...
void StateEstimation::position_update(const Distance &pos,
  ...
  std::printf("duration in micro seconds: %f\n", dur_us);
  last_update = Timestamp::now();
  constexpr float us_in_s = 1000000.0f;
  // predict new state based on old one
  ekf.f_xu[pos_i] = ekf.x_hat[pos_i] + 0.5 * dur_us * dur_us * ekf.x_hat[acc_i] / us_in_s / us_in_s;
  ...
  ekf.R[imu_i * DIM_OBSER + imu_i] = std::numeric_limits<float>::infinity(); // and its variance high 
